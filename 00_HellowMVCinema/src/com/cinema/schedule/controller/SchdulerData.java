...
public class SchdulerData extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("1"+theaterParameter);
		System.out.println("2"+moviesParameter);
		System.out.println("3"+dateParameter);
		
		if(!theaterParameter.equals("")) {
			theaterParameter = " AND R.TID IN("+theaterParameter+") ";
		}
		if(!moviesParameter.equals("")) {
			moviesParameter =  " AND S.MID IN("+moviesParameter+") ";
		}
		if(!dateParameter.equals("")) {
			dateParameter =  " AND TO_CHAR(S.TIME, 'yyyy-mm-dd hh24:mi:ss') LIKE '%"+dateParameter+"%' ";
		}
		...
		if(theaterParameter != null) {
			...
			query ="SELECT SHID, T.TID, R.RID, SID, T.NAME, R.NAME RNAME, R.TYPE RTYPE, M.NAME MNAME, M.GRADE, M.POSTER, (REGEXP_COUNT(SEAT,',')-REGEXP_COUNT(SEAT,',null')-REGEXP_COUNT(SEAT,',0')) SCNT,  REGEXP_COUNT(SEAT,',1') SLCNT, TO_CHAR(M.RELDATE, 'yyyy-mm-dd hh24') MRELDATE, TO_CHAR(S.TIME, 'yyyy-mm-dd hh24:mi:ss') STIME, TO_CHAR(S.ENDTIME, 'yyyy-mm-dd hh24:mi:ss') SENDTIME, S.SEAT"
					 + " FROM THEATER T, ROOM R, SCHEDULE S, MOVIEDATA M WHERE T.TID = R.TID AND R.RID = S.RID AND M.MID = S.MID "+theaterParameter+" " +moviesParameter+ " "+dateParameter+" ORDER BY S.TIME";
