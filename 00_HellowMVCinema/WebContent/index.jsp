	   ...
	   $.ajax({
	      ...
	      success:function(data){
	         ...
	         mitem += "</div>"
	            ...
	            /*--------------여기서부터 바뀌었습니다 ----------------------------------------------------------*/
	         mitem += "<div class='story'><h3>줄거리</h3>"+data.story+"</div>";
	         mitem += "<div class='trailer'><h3>예고편</h3>"+data.trailer+"</div>";
	         ...
	         success:function(data){
	            ...
	            if("null"==data[0]){ //댓글이 없다. 
	               ...
	               for(var i in data){
	                  ...
	                  <%if(userSession!=null){%>
	                  console.log("로그인해서 널은 아니지");
	                     if(review.usid==<%=userSession.getUsid()%>){
	                        
//	                        html +="<div class='edit-hide'><div class='id'>"+review.id+"</div><input type='number' name='star' min='1' max='5' value='"+review.grade+"'>";
	                        html +="<div class='edit-hide'><div class='id'>"+review.id+"</div>";
	                        html += "<span id='star2'><input type='hidden' name='starScore2' value='"+review.grade+"' />";
	                        html += "<span class='fa fa-star khchecked fzz'></span><span class='fa fa-star khchecked fzz'></span><span class='fa fa-star khchecked fzz'></span><span class='fa fa-star fzz'></span><span class='fa fa-star fzz'></span><div class='names2'></div></span>";
	   ...
	   $(function(){
	            ...
	            $.ajax({
	               ...
	               success:function(data){
	                  ...
	                  for(var i in data){
	                     ...
	                     //html += "<li><div class='viewReview'><div class='id'>"+review.id+"</div><span class='regdate'>"+review.regdate+"</span>&nbsp; <span class='star'>"+starGrader+"</span>\r\n<span class='content'>"+review.content+"</span>";
	                     html += "<li><div class='reviewr_img'><img src=/cinema/data/profile_m.png width=50/></div> <div class='viewReview'><div class='id'>"+review.id+"</div><span class='regdate'>"+review.regdate+"</span> <span class='star'>"+starGrader+"</span>\r\n<span class='content'>"+review.content+"</span>";
	                     <%if(userSession!=null){%>
	      ...
	      $("div.movieReviewList").on("click","button.btn-update",function(){ 
	         console.log("수정버튼");
	         $(this).parent("div").next("div").removeClass("edit-hide").addClass("edit-container");
	         $(this).parent("div").addClass("hide");
	         $(this).parent("div").prevAll().css("display","none");
	      });
	      
	       //리뷰 수정에서 별 ~~ 평점 mouseenter이벤트
	       $("div.movieReviewList").on("mouseenter","span.fzz",function(){
	            $(".fzz").removeClass("khchecked");
	            console.log("dddddd");
	            
	            $("[name=starScore2]").val($(this).index());
	            $(".names2").text(starName[$(this).index()-1]);   
	            for(var i =0; i< $(this).index(); i++ ){
	               $(".fzz").eq(i).addClass("khchecked");
	            }
	         });
	      ...
	      $("div.movieReviewList").on("click","button.cancel",function(){ //한줄평 수정 화면에서 수정 취소 버튼을 눌렀다. 취소 버튼 클릭 이벤트 
	         ...
	         $(this).parent("div").prev("div").prevAll().css("display","block");
	         $(this).parent("div").prev("div").removeClass("hide");
	      ...
	      $("div.movieReviewList").on("click","button.updateend",function(){
	         var review = {mid:movieMid,rvid:$("input[name=rvid]").val(),star:$("input[name=starScore2]").val(),content:$("textarea.review-update").val()};
	         $.ajax({
	...
	$.ajax({
		...
		success:function(data){
			...
			var mitem = "<div class='view_detail'>";
				...
				mitem += 			"<div><h3>영화</h3> <div></div></div>";
